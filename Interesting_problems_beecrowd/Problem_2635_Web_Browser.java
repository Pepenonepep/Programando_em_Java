...
public class Problem_2635_Web_Browser {

    public static int palavrasSugeridas(String palavrasPesquisadas[], String palavraCT, int tampesquisadas) {

        int qntPalavras = 0;

        for (int k = 0; k < tampesquisadas; k++) {

            String palavraP = palavrasPesquisadas[k];

            int j = 0;

            while (j < palavraCT.length() && j < palavraP.length() && palavraCT.charAt(j) == palavraP.charAt(j)) {
                j++;
            }
            if (j == palavraCT.length()) {
                qntPalavras++;
            }
        }
    ...
    public static int MaiorPalavraEncontrada(String palavrasPesquisadas[], String palavraCT, int tampesquisadas) {
        int maiorPalavra = 0;
        ...
        for (int k = 0; k < tampesquisadas; k++) {
            ...
            if (j == palavraCT.length() && maiorPalavra < palavraP.length()) {
                maiorPalavra = palavraP.length();
            }
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (sc.hasNextInt()) {
            int n = sc.nextInt();
            String[] palavrasPesquisadas = new String[n];
            for (int i = 0; i < n; i++) {
                palavrasPesquisadas[i] = sc.next();
            }
            int q = sc.nextInt();
            for (int j = 0; j < q; j++) {
                String palavraCT = sc.next();
                int qntPalavras = palavrasSugeridas(palavrasPesquisadas, palavraCT, n);
                int maiorPalavra = MaiorPalavraEncontrada(palavrasPesquisadas, palavraCT, n);
                if (qntPalavras == 0 && maiorPalavra == 0) {
                    System.out.println("-1");
                } else {
                    System.out.println(qntPalavras + " " + maiorPalavra);
                }
